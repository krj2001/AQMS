        ...
        while($deviceRow = mysqli_fetch_assoc($deviceListResult)){
            ...
            if(mysqli_num_rows($sensorListResult) > 0){
                while($row=mysqli_fetch_array($sensorListResult))
                {
                    ...
                    $lab=$row['lab_id'];
                    $dateTime = "";
                    $sql="select * from sampled_sensor_data_details_MinMaxAvg where device_id='$deviceId' and parameterName='$parameterName' order by id desc";
                        ...
                        else
                        {
                            ...
                            if($res){
                                foreach($DeviceSensorAQIList as $id => $val){
                                    ...
                                    $insertSensorAqiResult = mysqli_query($conn,$insertSensorAqi);
