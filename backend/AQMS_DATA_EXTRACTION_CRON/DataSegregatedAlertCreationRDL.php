...
function InsertDataToAlertTable($conn,$a_date,$a_time,$companyCode,$deviceId,$sensorId,$sensorTag,$alertTypes,$val,$Message,$severity,$status,$statusMessage,$alarmType,$current_time,$notificationStatus,$lab_id){
    ...
    $sql = "UPDATE alert_crons set triggeredAlertFlag='0' where sensorId='$sensorId' and alertCategory = '$alertcategory' order by id desc LIMIT 1";
    $res=mysqli_query($conn, $sql) or die(mysqli_error($conn)); 
    
    $sql_query = "INSERT INTO `alert_crons`(`a_date`, `a_time`, `companyCode`, `deviceId`, `sensorId`, `sensorTag`, `alertType`,`value`, `msg`, `severity`, `status`,`statusMessage`,`alarmType`,`alertCategory`,`triggeredAlertFlag`) VALUES 
                      ('$a_date','$a_time','$companyCode','$deviceId','$sensorId','$sensorTag','$alertTypes','$val','$Message','$severity','$status','$statusMessage','$alarmType','$alertcategory','$triggeredAlertFlag')";
