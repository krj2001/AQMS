                ...
                if($key !== "DATE" && $key !== "DEVICE_ID" && $key !== "MODE" && $key !== "TIME" && $key !== "ACCESS_CODE"){
                    ...
                    if($rawValue != ""){
                        ...
                        if($MODE == 2){
                            ...
                            $fetchSegregatedDataCountSql = "select * from segregatedValuesRDL";
                            ...
                            if($rowCount > 10000){
                                ...
                                $segregatedValuesSql = "INSERT INTO `segregatedValuesRDL`(`device_id`, `sensorTag`, `deviceMode`, `accessCode`, `val`,`scaledVal`,`current_date_time`, `upload_date_time`)
                        ...
                        // if($MODE == 4){
                        //     echo "insert bumptest mode";
                        //     $scaledValue= scalingValue($conn,$deviceId,$sensorTagName,$rawValue);
                            
                        //     $segregatedValuesSql = "INSERT INTO `segregatedBumptestValues`(`device_id`, `sensorTag`, `deviceMode`, `accessCode`, `val`,`scaledVal`,`current_date_time`, `upload_date_time`)
                        //                     VALUES ('$deviceId','$sensorTagName','$mode','$accessCode','$rawValue','$scaledValue','$time_in_24_hour_format_currentTime','$uploadDateTime')";
                        //     // echo $segregatedValuesSql."</br>";              
                        ...
                        // $fetchDeviceModeResult = mysqli_query($conn, $fetchDeviceModeSql);
                        
                        // if(mysqli_num_rows($fetchDeviceModeResult)>0){
                        //     $result = mysqli_fetch_assoc($fetchDeviceModeResult);
                        //     if($result['deviceMode'] == "enabled" && $MODE === '2'){
                        //         echo "Enabled mode"."<br>";
                        //         $scaledValue= scalingValue($conn,$deviceId,$sensorTagName,$rawValue);
...
$data_table="aqmi_json_dataRDL";//fetching data from main table where data is pushed
//$moving_window_width=15*60;//in seconds
$moving_window_width=5*60;//in seconds     fetching last 2mins data
