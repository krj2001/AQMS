...
if(mysqli_num_rows($getSensorListResult) > 0){
    while($sensorRow = mysqli_fetch_assoc($getSensorListResult)){
        ...
        $getUniqueSensorLastAvgValSql = "select  avg(avg_val) as AvgVal FROM `sampled_sensor_data_details_MinMaxAvg`  WHERE sample_date_time  >= DATE_SUB('".$time_in_24_hour_format_currentTime."',INTERVAL 15 MINUTE) and sensor_id = $sensor_id";
