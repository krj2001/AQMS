...
function getValuesForAParameter($conn,$data_table,$parameteInfo,$moving_window_width,$mode)
{
   ...
   /* echo $CUR_DATE ." ".(date("H")).":". $moving_window_start_p_min.":".$moving_window_start_p_s."</br>";
    ...
    $selectors=array($device_id_sel,$mode_sel);
    $query_filter="";
...
function extractAQMSParameterData($conn,$data_table,$moving_window_width,$mode)
{
    //where sensorNameUnit = 'PM10'
         $deviceListSql="SELECT * FROM devices where deviceMode = 'enabled'";
        //$deviceListSql="SELECT * FROM devices where deviceMode = 'enabled' order by id desc";
        ...
        if(mysqli_num_rows($deviceListResult) > 0){
            ...
            while($deviceRow = mysqli_fetch_assoc($deviceListResult)){
                // $sensorListSql = "select * from sensors where deviceId = ".$deviceRow['id']." and id = 46 order by id asc";
                ...
                if(mysqli_num_rows($sensorListResult) > 0){
                    while($row=mysqli_fetch_array($sensorListResult))
                    {
                        ...
                        /** computing twa between mentioned time begin **/
                            ...
                            // $userEndTime = "6:00 PM";
                            $userStartTime = $row['twaStartTime'];
                    ...
                    echo "No sensors for ".$deviceRow['deviceName']; 
                }
...
//                 $sql="select * from Aqi_values_per_device where companyCode='$companyCode' and locationId=$location and branchId=$branch and buildingId=$building and floorId=$floor and labId=$lab and deviceId=$deviceId and sampled_date_time='$dateTime'";
...
//                 {
//                      $sql="update Aqi_values_per_device set AqiValue='$aqiValue' where companyCode='$companyCode' and locationId='$location' and branchId='$branch' and buildingId='$building' and floorId='$floor' and labId='$lab' and deviceId='$deviceId' and sampled_date_time='$dateTime'";
//                 }
//                 else
//                 {
//                     $sql="insert into Aqi_values_per_device(companyCode,locationId,branchId,buildingId,floorId,labId,deviceId,AqiValue,sampled_date_time) values('$companyCode','$location','$branch','$building','$floor','$lab','$deviceId','$aqiValue','$dateTime')";
//                 }
...
function setParameterDataForCurrentWindow($conn,$parameter_data,$sensor_id,$device_id)
{
    ...
    $sql="select * from sampled_sensor_data_details_MinMaxAvg where parameterName='".($parameterName)."' and sensor_id=".$sensor_id." and sample_date_time='$sample_date_time'";
   // echo $sql;
    $res=mysqli_query($conn,$sql) or die(mysqli_error($conn));
    if(mysqli_num_rows($res)>0){
        $sql_query="update sampled_sensor_data_details_MinMaxAvg set last_val='$last_val',max_val='$max_val',min_val='$min_val',avg_val='$avg_val',alertType='$alertType', sevierity='$sevierity' ,param_unit='$param_unit' where parameterName='".($parameterName)."' and sensor_id=".$sensor_id." and sample_date_time='$sample_date_time'";
    }
    else
    {
       $sql_query="insert into sampled_sensor_data_details_MinMaxAvg(device_id,sensor_id,parameterName,last_val,max_val,min_val,avg_val,sample_date_time,param_unit,alertType,sevierity) values($device_id,$sensor_id,'$parameterName','$last_val','$max_val','$min_val','$avg_val','$sample_date_time','$param_unit','$alertType','$sevierity')";
    }
    $res=mysqli_query($conn, $sql_query) or die(mysqli_error($conn));
}
...
$device_mode="2";
 ...
 print_r(extractAQMSParameterData($mysqli,$data_table,$moving_window_width,$device_mode));

$data_table="sampled_sensor_data_details_MinMaxAvg";
