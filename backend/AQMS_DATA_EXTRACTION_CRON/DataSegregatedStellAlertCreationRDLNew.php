...
// not needed $getSensorListSql = "select  DISTINCT sensor_id FROM `sampled_sensor_data_details_MinMaxAvg`  WHERE sample_date_time  >= DATE_SUB('".$time_in_24_hour_format_currentTime."',INTERVAL 2 MINUTE)";

//$getSensorListSql = "select  DISTINCT sensor_id FROM `sampled_sensor_data_details_MinMaxAvg`"; commented orginal query

$getSensorListSql = "select sensor_id FROM `sampled_sensor_data_details_MinMaxAvg` where sensor_id = '46'"; //used for testing the query
...
if(mysqli_num_rows($getSensorListResult) > 0){
    while($sensorRow = mysqli_fetch_assoc($getSensorListResult)){
        ...
        $getUniqueSensorLastAvgValSql = "select  avg(avg_val) as AvgVal FROM `sampled_sensor_data_details_MinMaxAvg`  WHERE sample_date_time  >= DATE_SUB('".$time_in_24_hour_format_currentTime."',INTERVAL 1 MINUTE) and sensor_id = $sensor_id";
...
function InsertAlertCronData($conn,$a_date,$a_time,$companyCode,$deviceId,$sensorId,$sensorTag,$alertTypes,$val,$Message,$severity,$status,$statusMessage,$alarmType,$current_time,$sensorStatus,$notificationStatus,$hooterRelayStatus,$lab_id){
    ...
    if($sensorStatus == 0){
        ...
        $alertSql = "select * from  alert_cronsOLD2 where companyCode='$companyCode' and deviceId='$deviceId' and sensorTag='$sensorTag' and status='0'  order by id desc Limit 1";
