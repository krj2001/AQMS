...
class sendAlertMailAndAlertNumber{
    ...
    function __construct($sendingMailInfo, $senderEmail, $password) {       
    ...
    while($sensorRow = mysqli_fetch_assoc($getSensorListResult)){
        ...
        if(mysqli_num_rows($getUniqueSensorLastAvgValResult)>0){
            while($sensorLastRow = mysqli_fetch_assoc($getUniqueSensorLastAvgValResult)){
                ...
                $alertSentenceCase = array("outOfRange"=>"Out Of Range","Critical"=>"Critical","Warning"=>"Warning","NORMAL"=>"Normal");
                
                echo "hello abhishek $sensor_id  ".$paramterName."  alert:".$alertSentenceCase[$alert]."<br>";
                echo"hii";
                ...
                if($sensor_id != ""){
                    ...
                    $getLastSensorValuesSql = "SELECT * FROM `segregatedValues` where sensorTag = '$sensorTag' and device_id = '$deviceId' order by id desc limit 1";
                        ...
                        if(mysqli_num_rows($getLastSensorValuesResult) > 0){
                            ...
                            $insertIntoSampleDetailsSql = "INSERT INTO `sampled_sensor_data_details_MinMaxAvg`(`device_id`, `sensor_id`, `parameterName`, `last_val`, `max_val`, `min_val`, `avg_val`, `sample_date_time`, `current_date_time`,`alertType`, `alertStandardMessage`,`sevierity`,`param_unit`) VALUES
                                ('$deviceId','$sensor_id','$paramterName','$lastVal','$max','$min','$avg_val','$upload_date_time','$time_in_24_hour_format_currentTime','$alert','$alertSentenceCase[$alert]','$sevierity','ppm')";  
...
function alertMessage($conn,$sensorTag,$deviceId,$val,$uploadDate,$uploadTime){
    ...
    if($normalStatus === 1){
        ...
        if($alarmType == "UnLatch"){ //COMMENTED FOR TIME BEING IF ONLY LATCHED ALARM UPDATE
                ...
                $sql = "UPDATE alert_crons set status='$alertStatus[0]',statusMessage='$alertStatusMessage[0]',severity='$severityStatus', Reason='$reason',triggeredAlertFlag='0' where sensorId='$sensorId'";
                $res=mysqli_query($conn, $sql) or die(mysqli_error($conn)); 
                
                //update relay output to enable if data is normal for unlatched and if sensor hooter relay is enabled
                if($hooterRelayStatus == 1){
                    $updateLabHooter = "UPDATE lab_departments SET 	labHooterStatus = 0 WHERE id = '$lab_id'";
            ...
            //update relay output to enable if data is normal for latched
                ...
                $sql = "UPDATE alert_crons set triggeredAlertFlag='0' where sensorId='$sensorId' and alertCategory = '1'";
                $res=mysqli_query($conn, $sql) or die(mysqli_error($conn)); 
                // updateRelayOutputStatus($conn,$a_date,$a_time,$companyCode,$deviceId,$sensorId,$sensorTag,$alertTypes,$severity,$statusMessage,$hooterRelayStatus);
        }
    }
...
function InsertAlertCronData($conn,$a_date,$a_time,$companyCode,$deviceId,$sensorId,$sensorTag,$alertTypes,$val,$Message,$severity,$status,$statusMessage,$alarmType,$current_time,$sensorStatus,$notificationStatus,$hooterRelayStatus,$lab_id){
   ...
   $alertSentenceCase = array("outOfRange"=>"Out Of Range","Critical"=>"Critical","Warning"=>"Warning","NORMAL"=>"Normal");
    ...
    if($sensorStatus == 0){
        ...
        $alertSql = "select * from  alert_crons where companyCode='$companyCode' and deviceId='$deviceId' and sensorTag='$sensorTag' and status='0' and alertCategory = '1' and triggeredAlertFlag = '1' order by id desc Limit 1";
        ...
        if($tot_rows > 0){
            if($alertTypes  == $alertRow['alertType']){
                if($severity == $alertRow['severity']){
                    ...
                    InsertDataToAlertTable($conn,$a_date,$a_time,$companyCode,$deviceId,$sensorId,$sensorTag,$alertTypes,$val,$Message,$severity,$status,$statusMessage,$alarmType,$current_time,$notificationStatus,$lab_id,$hooterRelayStatus);   
                }
            }
            else{
...
function InsertDataToAlertTable($conn,$a_date,$a_time,$companyCode,$deviceId,$sensorId,$sensorTag,$alertTypes,$val,$Message,$severity,$status,$statusMessage,$alarmType,$current_time,$notificationStatus,$lab_id,$hooterRelayStatus){
       
    $triggeredAlertFlag = 1;
    $alertcategory = 1;
    
    echo "HooterStatus:".$hooterRelayStatus;
    ...
    //code to update lab hooter if there is alert enable lab hooter and diable if it is normal which ia implemented when condition is normal
    if($hooterRelayStatus == 1){
   ...
   //if sensor is enabled with email notification only then mail has to be sent
    ...
    if($notificationStatus == 1){
        ...
        $defaultEmail = "developer5@rdltech.in";
        $defaultPassword = "Ce=UmdQ7KFmV7nZ%%";     
            ...
            $getSensorDetails = "SELECT customers.customerId, customers.customerName, locations.stateName, branches.branchName, facilities.facilityName, buildings.buildingName, floors.floorName, lab_departments.labDepName, devices.deviceName, sensors.sensorTag FROM customers 
                ...
                if($alertTypes == "Warning"){
                    ...
                    if($returnedData[0] == 1){
                        ...
                        $body = "Alert Details as follows";
                        $subject = $alertTypes." ".$sensorTag;
                    }
                }
                
                // if($alertTypes == "Out Of Range"){  // Commented on 25-02-2023
                ...
                $sendingMailInfo['bodyMessage'] ="<b>". $body."</b>"."<br>"."<br>".
                                                    "<tr>
                                                        <td>Message : ".$mes."</td>
                                                    </tr><br>
                                                    <tr>
                                                        <td>Sensor Tag : ".$sensorTag."</td>
                                                    </tr><br>
                                                    <tr>  
                                                        <td>Device : ".$deviceName."</td>   
                                                    </tr><br>
                                                    <tr>  
                                                        <td>Zone : ".$labDepName."</td>   
                                                    </tr><br>
                                                    <tr>  
                ...
                $credentials = "SELECT * FROM `customers` where customerId = '$companyCode' LIMIT 1";
                $result = mysqli_query($conn, $credentials);
                $rowData = mysqli_fetch_assoc($result);
                $customerEmail = $rowData['systemEmail'];
                $customerPassword = $rowData['systemPassword'];
                
                $senderEmail = (!empty($customerEmail)) ? $customerEmail : $defaultEmail;
                $password = (!empty($customerPassword)) ? $customerPassword : $defaultPassword;
                
                echo $senderEmail.$password;
                $mailInfo = new sendAlertMailAndAlertNumber($sendingMailInfo, $senderEmail, $password);
